...
Description:
    ...
    DBAOps.trace.SaLoginHistory. Purges records older than the configured retention
    period in the same run. Interim audit control during sa remediation while sa
    cannot yet be disabled.
...
Steps:
    1. Capture sa XE events  — reads XE file target, deduplicates, inserts new rows
...
IF EXISTS (SELECT 1 FROM msdb.dbo.sysjobs WHERE [name] = N'DBAOps - Capture - SaActivity')
BEGIN
    EXEC msdb.dbo.sp_delete_job
        @job_name               = N'DBAOps - Capture - SaActivity',
...
EXEC @ReturnCode = msdb.dbo.sp_add_job
    ...
    @description           = N'Reads the DBAOps_SaActivityMonitor XE file target and inserts new events into DBAOps.trace.SaLoginHistory. Purges records older than 90 days. Interim control during sa remediation.',
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep
    ...
    @subsystem           = N'TSQL',
    @command             = N'EXEC [trace].[CaptureSaActivity] @XELogPath = N''$(XELogsPath)sa_activity*.xel'';',
...
-- Consider extending to 180 while sa remediation is active:
-- EXEC [trace].[CleanupSaActivity] @RetentionDays = 180;
